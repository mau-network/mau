...
export class Client {
  ...
  private async ensureReady(): Promise<string> {
    ...
    if (!this.authenticated) {
      await this.pinCertificate(this.resolvedAddress);
  ...
  /**
   * Verify the server's TLS certificate embeds the expected PGP fingerprint.
   *
   * Only runs in Node.js — browsers do not expose TLS certificate details via
   * any standard Web API so we skip pinning there gracefully.
   *
   * The server is expected to present a certificate generated by
   * `generateCertificate()` (crypto/pgp.ts), which encodes the PGP fingerprint
   * in the Subject CN and as a `pgp:<fingerprint>` Subject Alternative Name URI.
   */
  private async pinCertificate(address: string): Promise<void> {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        const socket = tls.connect({ host, port, rejectUnauthorized: false }, () => {
          ...
          // Certificate CN may be the raw fingerprint or "pgp:<fingerprint>"
          if (cn && normalizeFingerprint(cn.replace(/^pgp:/i, '')) === expectedFpr) {
            resolve(); return;
          }

          // SAN URI format: "URI:pgp:<fingerprint>"
          const sanMatch = san.match(/URI:pgp:([0-9a-fA-F]+)/i);
          if (sanMatch && normalizeFingerprint(sanMatch[1]) === expectedFpr) {
            resolve(); return;
          }

          reject(new IncorrectPeerCertificateError());
        });

        socket.on('error', reject);
        setTimeout(() => { socket.destroy(); reject(new Error('TLS cert check timeout')); }, 5000);
      });
    } catch (err) {
      // Re-throw identity mismatches; swallow connection/TLS-unavailable errors
      if (err instanceof IncorrectPeerCertificateError) throw err;
    }
  }
